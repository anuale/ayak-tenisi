...
export function ScoreEntry({
  ...
  function calculateWinner() {
    ...
    for (let s = 1; s <= TOTAL_SETS; s++) {
      ...
      const target = s === 5 ? TIEBREAK_POINTS : SET_POINTS;
      if (a >= target && a - b >= 2) aWins++;
      else if (b >= target && b - a >= 2) bWins++;
      else if (a > 0 || b > 0) {
        setError(`Set ${s}: Bir set kazanmak için ${target} sayıya ulaşmalı ve 2 fark olmalı.`);
        return null;
      }
    ...
    if (aWins < 3 && bWins < 3) {
      setError("Maçın bitmesi için bir takım 3 set kazanmalı.");
      return null;
    }
      ...
      <main className="flex-1 px-6 pt-6 flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-4">
          {Array.from({ length: TOTAL_SETS }).map((_, i) => {
            const setNum = i + 1;
            const isTiebreak = setNum === 5;
            const target = isTiebreak ? TIEBREAK_POINTS : SET_POINTS;

            return (
              <div
                key={setNum}
                className={`glass-surface border rounded-xl p-4 ${
                  isTiebreak ? "border-team-b/20" : "border-border/50"
                }`}
              >
                <div className="flex items-center gap-2 mb-3">
                  ...
                  {isTiebreak && (
                    <span className="text-[10px] bg-team-b/10 text-team-b px-2 py-0.5 rounded-full">
                      TIE-BREAK
                    </span>
                  )}
                ...
                <div className="flex items-center gap-3">
                  <input
                    className="w-16 h-14 bg-surface-high border border-border rounded-xl text-center text-2xl font-heading font-bold text-foreground focus:border-primary focus:outline-none"
                    placeholder="0"
                  ...
                  <input
                    className="w-16 h-14 bg-surface-high border border-border rounded-xl text-center text-2xl font-heading font-bold text-foreground focus:border-team-b focus:outline-none"
                    placeholder="0"
